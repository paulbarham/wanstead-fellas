...
| ⚠️ 9 new players on default ratings | ⚠️ | chat 30 Aug 2026 | Every joiner since 17 Jul sits on `overall_rating = 7`, untouched, and they're playing — Marshall Winter 6 apps, Ollie Hoad 3, Gavin Fulcher 2. Recent squads carry ~2 unrated per team. A default of 7 is mid-scale so it's invisible to rating-sum balancing while being potentially far wrong, i.e. the balancer optimises a fictional number for a fifth of the pitch. **Admin action, not a code fix** — blocks meaningful re-measurement. |
