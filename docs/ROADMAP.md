...
_Last updated: 2026-09-06 (Shipped History tab "Show lineups" reveal — every archived match now has expandable per-team rosters with captain + formation + DEBUT badges · SW v55. Earlier: profile dupe cleanup · mig 089 · squad ratings audit · mig 088 · push adoption drive · SW v54.)_
...
| ⚠️→✅ 19 players on default ratings | ✅ | commit pending 1 Sep 2026 · mig `088` | **Audit landed** — 18 of the 19 default-rated profiles populated from the 30 Aug worksheet (primer 27); Emmanuel deliberately skipped (surname missing on profile). Set all six `card_*` stats + `overall_rating` for each, plus `preferred_position_primary` for the 5 who had none (Marshall Winter → ATT · Gavin Fulcher · Bodhi Quinlan-May · James Lightbody · Daniel K → all MID). Neil Perry (GK) got matching `gk_*` values in positional mapping alongside card. **Overall revisions** to already-rated players: Paul Finch 7→8, Mark Pearson 6→7, Peter May 8→7 (over), Noah Higgins 9→8 (over). Left unchanged after review: Callum Finch, Michael Farley, Pete Healey, Gary Edwards, Rory Wilson. Whole exercise logged as one row in the `decisions` table for audit trail. **Unblocks balancer v2 re-measurement** now that the draft is no longer seeing flat 7s for these 18 profiles. |
| RTG-spread post-draft swap pass | 🟢 | chat 1 Sep 2026 | **The immediate next-step fix.** Today the balancer only balances rating implicitly via the initial snake — no active pass minimises the sum-of-overall_rating spread across teams. The star and age passes can even move a high-OVR player onto an already-RTG-heavy team. Now that mig 088 has populated real card stats and OVR (range 5–9 instead of a wall of 7s), positional constraints will occasionally force uneven pairings that the snake can't fix. **Design**: a third post-draft pass in `enforceBalanceConstraints` (`src/lib/balance.ts`), same shape as the star / age passes. Greedy: find the pair (heaviest team, lightest team) and try a position-preserving swap that reduces the spread by at least 1 point without violating star cap or age spread; bail cleanly if no valid swap exists. Independent-latch pattern from the 30 Aug bug fix. Tolerance: aim to bring spread ≤3 points, matching the "±3 pts achievable with 3 swaps" pattern from the 18 Jun case study. ~half-day incl. unit tests. Ships as the cheap-first step before simulated annealing. |
...
| History tab — "Show lineups" reveal per match | ✅ | commit pending 6 Sep 2026 · SW v55 | Gap surfaced from user feedback: after a match moved to archive, no way to see who was on which team. The data was always there (`team_players` + `teams` rows persist forever) — pure UI gap. New `MatchLineups` component adds a collapsed "👥 Show lineups" toggle inside each expanded historical match card. On expand renders each team as a card with © captain, formation shape (from `team_formations` if set), roster (name + 🆕 DEBUT badge for debutants that night). All data eager-loaded in HistoryPage's initial fetch — no per-tap N+1 queries. Retroactively unlocks lineups for every match ever played (~200+ nights). |
...
| Match-report drafter agent (weekly) | ✅ | commit `91dc717` 6 Sep 2026 · migs `090` + `091` | Shipped, but as two Supabase edge functions on `pg_cron` rather than the originally-sketched GitHub Actions → GH issue flow — review happens in-app instead. **The timing is the whole point:** reports were historically hand-written around midnight Thursday, i.e. *before* `voting_windows.closes_at` (10:00 UK Fri), which is why MOTM and DOTD have never once appeared in one. The generator runs at **10:05 UK Friday**, after the ballot closes, so the awards are on file. `weekly-context` (09:00 UTC Fri) caches the week's PL/ELC results from football-data.org plus 2-3 non-football colour items via Claude `web_search` into a new `weekly_context` table. `generate-match-report` (10:05 UTC Fri) feeds `get_match_hooks(date)` + that context + the last 3 published reports (few-shot) to `claude-opus-5` with a **JSON-schema-enforced** output matching the existing section shape, and writes `results.status = 'draft'`. **Hard rule in the system prompt:** framing may be invented freely (Keegan, Little Britain, Drury, whatever the news gives it), pitch EVENTS may not — every factual claim traces to a hook. The predicted-vs-actual table is computed in code from `matches.predicted_order`, never written by the model. Draft pushes **Paul only**; the group push now fires on the *publish* flip, not the summary write. New mobile-first **Report Review** screen at `/admin/report-review` (per-section editing + Publish), plus a `ReportDraftBanner` on the Admin dashboard when something is waiting. |
| ⚠️ Prerequisite — `ANTHROPIC_API_KEY` in Supabase secrets | 🟢 | 6 Sep 2026 | **Blocks the first live run of `generate-match-report`.** The function is deployed and every guard path is verified, but the key is not set in Supabase → Edge Functions → Secrets, so the generate step returns `ANTHROPIC_API_KEY not configured` and `weekly-context` silently writes football results with zero colour items. Same billing prerequisite as the **Cost + rate-limit budget guard** row above — needs a pre-paid credit balance, which is what killed the WC26 cup-audit agent. Set the secret before Friday and the whole chain runs unattended. |
| Lock down the report cron callers | ✅ | commit `35abef3` 6 Sep 2026 · mig `092` | `call_weekly_context` / `call_generate_match_report` are `SECURITY DEFINER` in `public`, so PostgREST exposed them at `/rest/v1/rpc/` with EXECUTE granted to PUBLIC + anon. Both resolve their endpoint from vault themselves, so the URL was never a gate — **confirmed live: an anon POST returned HTTP 204, i.e. it actually fired**, an anonymous trigger for Anthropic spend. Mig 092 revokes anon/PUBLIC EXECUTE and adds an in-function guard (`session_user = 'postgres'` for cron, else `is_admin()`), keeping an admin regenerate button working. **Guard is on `session_user`, not `current_user`:** both functions are owned by `postgres`, so under SECURITY DEFINER `current_user` is `postgres` on *every* path including an anon web request — a `current_user` check reads like a guard and permits everyone. Measured on this DB: cron → `session_user` `postgres`, `request.method` null; PostgREST → `session_user` `authenticator`, `request.method` `POST`. |
| ⚠️ Same exposure on the other seven `call_*` functions | 🟢 | 6 Sep 2026 | `call_send_vote_notifications`, `call_mow_*`, `call_cup_results_sync`, `call_send_feature_announcement`, `call_admin_subs_chase_monthly`, `call_monthly_personal_review_notify` all still grant EXECUTE to anon. **They cannot take mig 092's guard as-is:** `call_send_vote_notifications` is invoked from inside the `results` / `voting_windows` triggers, where `session_user` is `authenticator` (an app write) and `is_admin()` is false for a delegate or the service-role edge function — adding the guard there would silently kill the match-night and results pushes. Needs a trigger-aware variant (e.g. allow when `pg_trigger_depth() > 0`). |
| Match report dry run | ✅ | commit `35abef3` 6 Sep 2026 | `generate-match-report` takes `dry_run: true` — generates as normal, returns the finished JSON under `would_write`, writes nothing and pushes nobody. Deliberately skips the anti-clobber guards so it can be pointed at a past night that already has a published report and diffed against it. Few-shot examples are drawn strictly before the target date, so a dry run never sees its own report. |
| Trim weekly-context football results | ✅ | commit `35abef3` 6 Sep 2026 | First run cached 54 scorelines (a full PL + ELC week) — a wall of numbers the report can't use and pure prompt bloat. Now capped at the 6 most newsworthy via a crude `headlineScore` (margin + goals, bonuses for a hiding / a thriller / a high-scoring draw, top flight breaks ties). Verified on real data: Southampton 5-1 Millwall, Man United 5-2 Ipswich, Stoke 4-0 Charlton, Millwall 4-0 Bolton, Chelsea 4-3 Brighton, West Ham 4-2 Wolves. The value of this feed is the colour items, not the scorelines. |
...
| Per-player monthly personal review | ✅ | commit pending 1 Sep 2026 · migs `086`+`087` · SW v53 · edge fn `monthly-personal-review-notify` | **RPC-first approach** instead of extending the JS report script — `monthly_player_summary(uuid, date)` computes everything from raw tables in one query: apps · goals · own-goals · MOTM/DOTD · W-D-L · **best game** (impact score = goals + MOTM×3 + DOTD×2) · **favourite teammate** (most shared fixtures) · **streak** (consecutive months with ≥1 app, computed via row_number + ordinal-month equality). **In-app page** `/profile/monthly/:month` with month-navigation prev/next, stat tile strip, Best Game section, Favourite Teammate card with avatar, streak card when ≥2 months. Zero-apps players see a "See you Thursday" fallback. **Push** at 07:15 UTC on 1st of month (mig 087 pg_cron → edge fn) — only pinges players who actually turned out (audience-filtered via `push_targets('club_news', player_ids_with_apps, false)`), no dead notifications for lurkers. Verified on real data: Sheridan Winter Aug 2026 → 4 apps, 6 goals, 2 MOTM, 4-0-0 unbeaten, best game 27 Aug (2 goals + MOTM), fav teammate Callum Finch (4 shared fixtures). Profile page gets a deep-link tile pointing at the most recent complete month. |
