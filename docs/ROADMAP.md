...
| Match of the Week — v1 | 🟡 | proposal 03 · chat 24 Jul · migration `062` · commit pending | Committee-approved. Auto-picked marquee fixture each week; scoreline pick; 5 pts exact · 3 pts result · 0 wrong. Shell + DB + score-picker UI + weekly/season leaderboards all shipped in this pass. Fixture pool + weekly picker + results poll wired via 3 edge fns (`mow-seed-fixtures`, `mow-pick-weekly`, `mow-fetch-results`) using football-data.org free tier — covers PL + Championship. Lower-league coverage (L1/L2) deferred until a free data source lands (see next row). Cron install + push notifications land in Chunk B (next session). |
