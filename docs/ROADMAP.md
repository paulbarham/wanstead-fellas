...
_Last updated: 2026-08-31 (App review batch: added rivalry cards under 🎬 Match narrative; monthly personal + annual PDF under 📆 Monthly round-up; new 📝 Governance & records theme with decisions log, subs FK, seed migration, public house-rules view)_
...
| Item | Status | Source | Notes |
|---|---|---|---|
| Decisions log — v1 admin-only | 🟢 | chat 31 Aug 2026 | New `decisions` table (id, decided_at, decided_by, category, summary, details, related_player_id nullable, **effective_from date, effective_to date null**, archived). Categories: `subs` / `house_rules` / `disputes` / `roster` / `finance` / `other`. Admin UI: reverse-chronological list with category chips + text search; "+ New" bottom sheet with dropdowns + one-line summary + expandable details + optional player tag + from/to date pickers. Append-only — no delete, no edit after 24h grace. **Effective_from/to particularly matters for subs**: half-sub arrangements can change season-to-season, shared subs (Felix/Guy) might become term-limited, mid-season joiners get prorated periods. ~half-day. |
| `club_subscriptions.decision_id` FK | 🟢 | chat 31 Aug 2026 | Link each sub row to its originating decision. "Why does Aaron pay £47.50?" becomes one join away instead of buried in `notes`. Migration adds nullable FK; backfill from existing notes where sensible. ~1 hour. |
