...
| Push audiences widened — teams_ready + vote_open | ✅ | chat 13 Aug 2026 · commit pending · edge-fn deploy pending admin approval | Trigger fired correctly on the 13 Aug publish (200 OK) but only 8/20 rostered fellas had a push subscription so most of the group heard nothing. Two policy fixes to `send-vote-notifications`: (1) `teams_ready` now fans out to every push_subscription just like `results`, making the line-up drop club-wide news; (2) `vote_open` still targets rostered players but now also includes admins even when off-roster, so the publisher gets the same confirmation ping. Data problem separately: 12 of the 20 rostered players for tonight have no push_subscription row at all — nudge them to hit Tonight → Enable notifications so they're covered from next week. |
...
| ~~Duplicate "match report is live" push at voting close~~ | ↩️ REVERTED (mig `075` shipped, mig `076` rolls it back). Admin preference (13 Aug 2026): keep the second buzz at voting close — even when the report went out early, the awards landing is genuinely new news and the extra ping acts as a nudge to open the app for the MOTM/DOTD reveal. Duplicate copy ("match report is live" twice) is the acceptable cost for now. Cleaner follow-up on the todo pile below. |
| 🟢 Awards push should have distinct copy from report push | 🟢 | chat 14 Aug 2026 | After mig 076 (revert), the 10am UK awards-published push re-uses the "📝 Match report is live" copy when a summary already exists — which is a duplicate of the ping the group got when the summary was written. Better UX: at voting-close time, if the summary is already on file, send a distinct "🏆 MOTM & DOTD winners announced" copy so the group knows the awards specifically have landed. Requires a new topic (e.g. `awards_only`) or a `hasSummary`-branched copy inside the existing `results` topic in `send-vote-notifications`, plus updating the trigger to pass it. Not urgent — the current duplicate is a mild-noise problem, not a broken flow. |
