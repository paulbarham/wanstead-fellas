...
| Match report dry run | ✅ | commit pending 6 Sep 2026 | `generate-match-report` takes `dry_run: true` — generates as normal, returns the finished JSON under `would_write`, writes nothing and pushes nobody. Deliberately skips the anti-clobber guards so it can be pointed at a past night that already has a published report and diffed against it. Few-shot examples are drawn strictly before the target date, so a dry run never sees its own report. |
| Trim weekly-context football results | ✅ | commit pending 6 Sep 2026 | First run cached 54 scorelines (a full PL + ELC week) — a wall of numbers the report can't use and pure prompt bloat. Now capped at the 6 most newsworthy via a crude `headlineScore` (margin + goals, bonuses for a hiding / a thriller / a high-scoring draw, top flight breaks ties). Verified on real data: Southampton 5-1 Millwall, Man United 5-2 Ipswich, Stoke 4-0 Charlton, Millwall 4-0 Bolton, Chelsea 4-3 Brighton, West Ham 4-2 Wolves. The value of this feed is the colour items, not the scorelines. |
