...
| ~~Duplicate "match report is live" push at voting close~~ | ✅ FIXED (commit pending · mig `075`). When admin writes the report before voting closes, both `results_notify_report_live_trg` (on summary write) and `voting_windows_notify_results_trg` (on awards published) fire the same `topic='results'` push a few hours apart. Tag coalescing hides most of it on iOS/Android but the buzz goes twice. Mig 075 teaches the awards-published trigger to short-circuit when `results.summary` already exists — the summary trigger has covered it. Awards-only fallback (admin publishes awards but hasn't written the report) still fires as normal. Case study: 13 Aug 2026 report shipped ~03:50 UTC — awards trigger due 09:00 UTC would have re-sent identical copy. |
