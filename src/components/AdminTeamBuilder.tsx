...
// Colours cycling through 4 team slots
  ...
  useEffect(() => {
    async function load() {
      const { data: avail } = await supabase
        .from('availability').select('player_id').eq('match_date', nextThursday)
  ...
  async function publish() {
    ...
    if (!matchId) {
      const { data: newMatch } = await supabase
        .from('matches')
        .insert({ match_date: nextThursday, format: draftTeams.length >= 3 ? '4-team' : '11v11', status: 'published' })
        .select().single()
      matchId = newMatch?.id
    } else {
    ...
    <div className="px-4 pt-4 pb-4">
      ...
      <div className="flex items-center justify-between mb-1">
        ...
        <span className="text-xs px-2 py-1 rounded-full" style={{ background: '#141414', color: '#888', border: '1px solid #2e2e2e' }}>
          {availablePlayers.length} signed up
        </span>
      </div>
      ...
      {/* Auto-balance button — primary action */}
      <button
        onClick={autoBalance}
